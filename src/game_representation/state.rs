...
impl Game {
    ...
    pub fn from_fen(fen: &str) -> Game {
        ...
        let color_to_move = match parts[1] {
            "w" => Color::White,
            "b" => Color::Black,
            _ => panic!("Color should be either w or b"),
        };
        ...
        if chars[0] == '-' {
            ...
            panic!("castling information is wrong");
        } else {
            for c in chars {
                match c {
                    'K' => {
        ...
        if parts[3] == "-" {
            ...
            en_passant = bitboard::field_repr_to_index(parts[3]);
        }

        let half_move_clock = parts[4]
            .parse()
            .expect("Half move clock provided is not a number");
        let full_move_clock = parts[5]
            .parse()
            .expect("Full move clock provided is not a number");
...
mod tests {
    ...
    fn fen_io_test() {
        ...
        assert_eq!(
            &Game::from_fen("r5kr/1pp1Qpp1/p1b1p3/R3P2p/3P4/1PN5/4NP1q/4K1R1 w - h6 0 21").to_fen(),
        ...
        assert_eq!(
            &Game::from_fen("r5k1/2Q2pp1/p1R5/3qp1p1/8/P2P2P1/1r2PP1P/5RK1 w - - 0 21").to_fen(),
        ...
        assert_eq!(
            &Game::from_fen("3r4/p1k2p2/1pn1b1p1/4p2p/2P5/B2B1P2/PP4PP/2KR4 w - h6 0 21").to_fen(),
        ...
        assert_eq!(
            &Game::from_fen("1k1r3r/1bq2ppp/pp3n2/8/P2N1P2/R1Nn4/1PP2QPP/5R1K w - - 0 21").to_fen(),
            "1k1r3r/1bq2ppp/pp3n2/8/P2N1P2/R1Nn4/1PP2QPP/5R1K w - - 0 21"
        );
        assert_eq!(
            &Game::from_fen("r6r/qQ3pp1/p2bpk1p/8/8/3B2P1/PP3P1P/3R1RK1 w - - 3 21").to_fen(),
            "r6r/qQ3pp1/p2bpk1p/8/8/3B2P1/PP3P1P/3R1RK1 w - - 3 21"
        );
        assert_eq!(
            &Game::from_fen("8/pp2rpkp/3R2p1/2r1pb2/8/1BP5/PP3PPP/5RK1 w - - 4 21").to_fen(),
            "8/pp2rpkp/3R2p1/2r1pb2/8/1BP5/PP3PPP/5RK1 w - - 4 21"
        );
        assert_eq!(
            &Game::from_fen("r6k/1ppq2pn/p2p3p/4pr2/6Q1/1PPPB2P/1P3PP1/R4RK1 w - - 1 21").to_fen(),
        ...
        assert_eq!(
            &Game::from_fen("1r1qr1k1/R2b1p1p/3p1pp1/2pP4/8/2PP2PP/3Q1PBK/5R2 w - - 2 21").to_fen(),
        ...
        assert_eq!(
            &Game::from_fen("2rr2k1/p5p1/1pn1pb1p/8/8/2P1B1PP/P4PB1/2RR2K1 w - - 0 21").to_fen(),
        ...
        assert_eq!(
            &Game::from_fen("rr4k1/3bppbp/p4np1/1pp5/4P3/1P1BBP2/P2KN1PP/2R4R w - - 0 21").to_fen(),
            "rr4k1/3bppbp/p4np1/1pp5/4P3/1P1BBP2/P2KN1PP/2R4R w - - 0 21"
        );
        assert_eq!(
            &Game::from_fen("r6r/ppp1k3/1n2b2b/4p2p/4PppP/5PP1/PPPNB2B/2KR1R2 w - - 1 21").to_fen(),
